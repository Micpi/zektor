class ThermoHaloCard extends HTMLElement {
    ...
    _handleTap(ev) {
        ...
        if (action === "more-info") {
            const entityId = tapAction.entity || this._config.temperature_entity;
            if (!entityId) {
                return;
            }
        ...
        if (action === "perform-action") {
            ...
            const target = tapAction.target && typeof tapAction.target === "object" ? tapAction.target : {};
    ...
    _render() {
        ...
        const setpoint = this._formatNumber(climateEntity?.attributes?.temperature, this._config.decimals);

        const mode = this._computeMode(climateEntity);

        const showSetpoint = mode.isOn && setpoint !== null;
        const haloClass = mode.halo === "heat" ? "halo-heat" : mode.halo === "cool" ? "halo-cool" : "halo-off";
        ...
        const heatHalo = isBaseAppearance
            ? "radial-gradient(circle, rgba(255, 154, 60, 0.14) 0%, rgba(255, 154, 60, 0) 72%)"
            : "radial-gradient(circle, rgba(255, 154, 60, 0.22) 0%, rgba(255, 154, 60, 0) 70%)";
        const coolHalo = isBaseAppearance
            ? "radial-gradient(circle, rgba(50, 184, 255, 0.14) 0%, rgba(50, 184, 255, 0) 72%)"
            : "radial-gradient(circle, rgba(50, 184, 255, 0.22) 0%, rgba(50, 184, 255, 0) 70%)";
