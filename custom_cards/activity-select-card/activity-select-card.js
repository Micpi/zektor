...
class ActivitySelectCardEditor extends HTMLElement {
  ...
  _render() {
    ...
    this._setFieldValue("cfg-inactive_color", c.inactive_color || "var(--card-background-color, #444)");
    ...
    this.shadowRoot.getElementById("btn-add").addEventListener("click", () => {
      const activities = [...this._activities, { name: "Nouvelle", icon: "mdi:help-circle", value: "" }];
  ...
  _renderActivities() {
    ...
    this._activities.forEach((act, idx) => {
      ...
      if (idx > 0) {
        ...
        upBtn.title = "Monter";
        upBtn.addEventListener("click", (e) => { e.stopPropagation(); this._moveActivity(idx, -1); });
      ...
      if (idx < this._activities.length - 1) {
        ...
        downBtn.addEventListener("click", (e) => { e.stopPropagation(); this._moveActivity(idx, 1); });
        actions.appendChild(downBtn);
      }
      const delBtn = document.createElement("ha-icon-button");
      delBtn.innerHTML = `<ha-icon icon="mdi:delete"></ha-icon>`;
      delBtn.title = "Supprimer";
      delBtn.addEventListener("click", (e) => { e.stopPropagation(); this._removeActivity(idx); });
      ...
      // Name
      const nameField = this._createTextField("Nom", act.name || "", (v) => this._updateActivity(idx, "name", v));
      ...
      // Value
      const valueField = this._createTextField("Valeur", act.value || "", (v) => this._updateActivity(idx, "value", v));

      // Icon color
      const colorField = this._createTextField("Couleur icône", act.icon_color || "", (v) => this._updateActivity(idx, "icon_color", v));
      ...
      showNameLabel.style.cssText = "display:flex;align-items:center;gap:8px;cursor:pointer;font-size:13px;color:var(--primary-text-color)";
...
window.customCards.push({
  ...
  preview: true,
  description: "Boutons ronds pour sélectionner une activité, entièrement configurable via l'interface graphique.",
});
